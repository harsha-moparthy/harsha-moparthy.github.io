...
description: >-
  ...
  from run-to-run noise — validated against synthetic agents whose true success rate is known.
stack: [Python, numpy, SQLite, Gemini, Ollama, GitHub Actions]
highlights:
  - value: "6.0%"
    label: measured A/A false-alarm rate
  - value: "100% / 25.5%"
    label: gate detection at 20- vs 5-point drops
  - value: "1e-12"
...
unit tests; agents mostly have vibes. The reason is nondeterminism: a single run proves nothing, so
naive "run it once and eyeball it" testing cannot tell a regression from noise — and CI gates built on
single runs either block good changes or wave through bad ones.
...
intervals can be validated the way numerical code should be:
...
If the machinery passes on agents where the answer is known, its reports on real agents deserve trust.
That validation-first stance is the project.
...
- **The gate** compares paired per-task success rates. Decision rule: block if the one-sided bootstrap
  ...
  θ and confidence level are explicit config, because a gate whose sensitivity you cannot state is
  theater.
...
| Gate detection: 5-point true drop (200 experiments) | **25.5%** — honest limit of n=5 × 20 tasks |
...
The 5-point-drop row is the important one: it is *measured evidence* that n = 5 over 20 tasks reliably
catches large regressions but only probabilistically catches subtle ones. A harness that doesn't tell you
its minimal detectable effect is marketing.
...
all 80 runs, and the gate rightly stayed open. Two lessons the harness itself surfaced:

1. prompt edits you fear are sometimes harmless, and only measurement tells you which;
2. the offline simulator's assumption (v2 skips tools 35% of the time) was wrong about this model —
   simulators are for exercising the *machinery*, and real verdicts require real runs.
...
- **A/B (20 tasks × 2 runs per arm):** v1 40/40, v2 40/40, step score 1.0 on every run; gate verdict *no
  regression detected* (Δ +0.000). The planted prompt degradation didn't fool the local model either,
...
- **Judge validation:** 96.7% agreement, kappa 0.933 on the 30 human labels — between the rule surrogate
  (86.7%) and Gemini (100%). The one error is a false positive on an output that names the pre-discount
  price while deflecting to sales; committed in the report rather than relabeled away.
...
published, including the regimes where n is too small to detect subtle regressions. The judge-validation
labels were written and labeled by me — stated plainly rather than laundered as independent annotation.
